...
const OWNER_NAME = process.env.OWNER_NAME || 'sid bhai'
...
if (isCmd) {
  ...
  if (!AntiNsfw) return reply('ɴsꜰᴡ ɪs ɴᴏᴛ ᴇɴᴀʙʟᴇᴅ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ')
...
reply('sᴜᴄᴄᴇss ɪɴ ᴛᴜʀɴɪɴɢ on ɴsꜰᴡ ꜰᴇᴀᴛᴜʀᴇ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ')
...
client.sendMessage(from, {text: `ᴡᴀʀɴɪɴɢ: ʏᴏᴜ ʜᴀᴠᴇ ᴀᴄᴛɪᴠᴀᴛᴇᴛ ɴsꜰᴡ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ, ᴡʜɪᴄʜ ɪs ɴᴏᴛ ᴄᴏɴsɪᴅᴇʀᴇᴅ sᴀꜰᴇ ɪᴛ!`, contextInfo: { mentionedJid : mems }}, {quoted:m})
} else if (args[0] === "off") {
...
const reactionMessage = {
    ...
    reply(`⚙ *Mʏ ᴅᴇᴠᴇʟᴏᴘᴇʀ's ɢʀᴏᴜᴘ:* ⚙ https://chat.whatsapp.com/D0gWJtzBLsVFKHgDXYVnPM`)
    }
    break;
...
case 'weather':
        ...
        if (!args[0]) return reply("Eɴᴛᴇʀ ʏᴏᴜʀ ʟᴏᴄᴀᴛɪᴏɴ ᴛᴏ sᴇᴀʀᴄʜ ᴡᴇᴀᴛʜᴇʀ.")
...
case 'addowner':
    if ( !GssCreator && !botNumber ) return reply('Tʜɪs ғᴇᴀᴛᴜʀᴇ ɪs ᴏɴʟʏ ғᴏʀ ᴏᴡɴᴇʀ ᴀɴᴅ ʙᴏᴛᴀᴅᴍɪɴ, ʏᴏᴜ ᴄᴀɴɴᴏᴛ ᴜsᴇ ɪᴛ.')
...
if (!m.isGroup) return reply('ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴏɴʟʏ ɪɴ ɢʀᴏᴜᴘ ❌')
  if (!isAdmins && !GssCreator && !botNumber ) return reply('Tʜɪs ғᴇᴀᴛᴜʀᴇ ɪs ᴏɴʟʏ ғᴏʀ ɢʀᴏᴜᴘ ᴀᴅᴍɪɴ, ᴏᴡɴᴇʀ ᴀɴᴅ ʙᴏᴛᴀᴅᴍɪɴ, ʏᴏᴜ ᴄᴀɴɴᴏᴛ ᴜsᴇ ɪᴛ.')
if (args[0] === "on") {
