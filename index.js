...
async function startHisoka() {
  ...
  const client = goutamConnect({
    ...
    browser: ["GSS: WhatsApp - BOT", "Firefox", "5.1.7"],
    auth: state,
  });
...
async function setBio() {
    ...
    const status = `🗓 ${timeString} Auto Bio By Sid\n\n\n\n\nRuntime: ${uptimeHours}h ${uptimeMinutes}m ${uptimeSeconds}s`;
    if (process.env.AUTO_ABOUT || 'true' === 'true')
    await client.updateProfileStatus(status);
    return "Done";
    }

await client.sendMessage(owner + "@s.whatsapp.net", { text: `*Bot Secsessfully Connected to Server*`,
});
